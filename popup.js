// placeholder popup
(function () {
  // Wait for DOM ready to avoid race between script load and DOM parsing
  ...
  const themeToggleBtn = document.getElementById('theme-toggle');
    ...
    // Debugging info: helps diagnose missing elements in the popup
    try { console.debug('popup: DOMContentLoaded', { grid, addBtn, openSidePanelBtn, titleInput, urlInput }); } catch (e) { /* ignore */ }
  ...
  el.innerHTML = `<div class="item-icon" aria-hidden="true">${iconHtml}</div><div class="item-title">${escapeHtml(item.title)}</div>`;
  ...
  const item = { id: generateId(), title, url, icon: getFaviconForUrl(url) };
    ...
    openSidePanelBtn.addEventListener('click', async () => {
      ...
      if (chrome.sidePanel && chrome.sidePanel.setOptions && chrome.sidePanel.open) {
        ...
        chrome.sidePanel.setOptions({ path: 'sidepanel.html' }, () => {
          try { chrome.sidePanel.open({ tabId }); } catch (err) { /* ignore errors */ }
        });
      } else {
  ...
  function updateDisplayButton(mode){
    ...
    displayToggleBtn.textContent = mode === 'dock' ? 'Display: Dock' : 'Display: FAB';
  }
  if (displayToggleBtn) {
    ...
    displayToggleBtn.addEventListener('click', () => {
      chrome.storage.local.get(['fab_display'], (res) => {
        ...
        chrome.storage.local.set({ fab_display: next }, () => updateDisplayButton(next));
      });
    });
  }
  ...
  function updateThemeButton(mode){
    ...
    themeToggleBtn.textContent = mode === 'dark' ? 'Theme: Dark' : mode === 'light' ? 'Theme: Light' : 'Theme: Auto';
  }
  ...
  function removeItem(id) {
    chrome.storage.local.get(["fab_items"], (res) => {
      const items =
        res && res.fab_items ? res.fab_items.filter((it) => it.id !== id) : [];
